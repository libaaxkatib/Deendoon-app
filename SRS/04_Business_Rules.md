...
| **Scope Baseline** | `01_Project_Overview.md` (Reopened v1.5) · `02_Business_Requirements.md` (Reopened v1.7) · `03_Functional_Requirements.md` (v1.19) |
...
| 1.11 | 2026-08-19 | **Manual Mobile-Money Subscription Payment Flow Amendment (Product Owner-approved decision), extending §12.** Added **BRL-092** (payment phone required, payment reference optional; Platform Payment Destination Number governance) — the Subscription Change Request row in the Validation Rules table updated to add Payment Phone (required) alongside the now-optional Payment Reference. No other Business Rule renumbered or changed; Storage Add-on Request rules (BRL-088/BRL-089) untouched — this amendment is scoped to Subscription Change Requests only, per Product Owner decision. Scope Baseline updated to cite `03` at v1.17. | Claude |
| 1.12 | 2026-08-20 | **Final UX direction for the Manual Mobile-Money Subscription Payment Flow (Product Owner-approved decision), amending BRL-092.** Added the two fixed operator USSD codes (SAALAM BANK `*799*32666663*5#`, EVC PLUS `*712*615514692*5#`) as Product-Owner-given application constants, explicitly distinct from the Platform Payment Destination Number governed by the same rule — the codes are not admin-configurable and not to be modified/added-to without a further Product Owner decision. Clarified that tapping a USSD button (or returning from the external dialer) never changes a Subscription Change Request's status. No schema, migration, or API change accompanies this — confirmed and stated explicitly in BRL-092's own Notes. Scope Baseline updated to cite `03` at v1.18. | Claude |
| 1.13 | 2026-08-20 | **Third payment operator added (Product Owner-approved decision), extending BRL-092.** Added E-DAHAB (`*712*625514692*5#`) as a third fixed operator USSD code alongside SAALAM BANK and EVC PLUS — same fixed-constant, non-admin-configurable, non-status-changing treatment as the first two. No other Business Rule changed; no schema, migration, or API change. Scope Baseline updated to cite `03` at v1.19. | Claude |
...
- **Rule Statement:** A Notification is created only as a direct, passive consequence of a qualifying event already generated by an owning module (Credit Limit Reached, Payment Received, Document Available, Collection Assignment, Reminder Sent, Promise to Pay Due, Professional Collection Request Update). This module never originates, schedules, or invents an event of its own.
...
- **Related Functional Requirements:** FR-058, FR-073, FR-075.
- **Notes:** Amended by Product Owner Decision (Business Owner Backend Completion, pre-Phase 5, 2026-08-07) to add Professional Collection Request Update as a seventh qualifying event source, alongside the approval of Module 7's amended scope (FR-072–076, `03_Functional_Requirements.md` v1.11). Collection Assignment (the original sixth source) is retired along with FR-041 (`03_Functional_Requirements.md` v1.8) and never fires — left in this list only because BRL-064's own text has not otherwise been renumbered.
...
### BRL-092 — Manual Mobile-Money Subscription Payment Flow (added, Revision History 1.11; amended, Revision History 1.12; amended, Revision History 1.13)
- **Purpose:** Define the payment-input fields for a Subscription Change Request, the governance of the Platform Payment Destination Number shown to the Business Owner, and the fixed operator USSD codes used to launch payment.
- **Applies To:** Request Subscription Plan Change (FR-078); Platform Payment Destination Number (FR-086).
- **Rule Statement:** A Subscription Change Request requires a `payment_phone` (the mobile-money number the payment is sent from, max 20 characters) — submission is rejected without one. A `payment_reference` (transaction reference, max 100 characters) is **optional**: the approved UX flow lets the Business Owner submit before the external mobile-money transaction has happened, so the reference may not exist yet. The Platform Payment Destination Number is a single, platform-level (not tenant-scoped) value; only the Deendoon Platform Administrator may set or change it, and every change is recorded in the Audit Trail. This rule applies only to Subscription Change Requests — Storage Add-on Requests are unaffected and continue to require a payment reference (BRL-088/BRL-089 unchanged). **Amended, Revision History 1.12 (final UX direction):** the Payment Instructions screen (`05` SCR-053) offers exactly two fixed, Product-Owner-given USSD codes, one per supported mobile-money operator — SAALAM BANK (`*799*32666663*5#`) and EVC PLUS (`*712*615514692*5#`) — launched via the device dialer. These two codes are **fixed application constants**, not sourced from `platform_payment_settings`, not admin-configurable, and not to be modified without a further Product Owner decision; they are a separate concept from the displayed Platform Payment Destination Number, which remains backend-driven exactly as before. Tapping either button, or returning from the external dialer app, never changes the Subscription Change Request's status — there is no reliable signal that a payment was completed, so the request remains Pending until the Deendoon Platform Administrator approves or rejects it (FR-084, unchanged). **Amended again, Revision History 1.13:** a third fixed operator, E-DAHAB (`*712*625514692*5#`), added alongside SAALAM BANK and EVC PLUS — same constant/non-configurable/non-status-changing treatment.
- **Trigger:** Business Owner submits a Subscription Change Request; Deendoon Platform Administrator sets or updates the destination number; Business Owner taps SAALAM BANK, EVC PLUS, or E-DAHAB on the Payment Instructions screen.
- **Result:** A request missing `payment_phone` is rejected with a validation error; a request missing `payment_reference` is accepted. The destination number is served to the Customer Mobile App live from the backend, never hardcoded client-side. Tapping a USSD button only opens the device dialer pre-filled with the fixed code — no request, subscription, or Audit Trail state changes as a result.
- **Exceptions:** A Business Owner attempting to set the destination number → rejected (Platform-Administrator-only). An empty destination number submission → rejected with a validation error. A failed dialer launch (e.g. no dialer app available) → an inline error is shown; the request remains Pending, unaffected either way.
- **Related Functional Requirements:** FR-078, FR-086.
- **Notes:** This rule does not change Storage Add-on Request validation, the approval/activation mechanics of BRL-085, or the one-pending-request rule of BRL-084 — it is additive to the existing Subscription Change Request input shape only. The two fixed USSD codes are a Customer Mobile App presentation detail (BRL-092/`05` SCR-053) with no corresponding backend field, migration, or API change — confirmed no `platform_payment_settings` or other schema change accompanies this amendment.
...
| Subscription Change Request *(added)* | Requested Plan (1 of 5 fixed plans), Payment Phone (required, max 20 chars, BRL-092) | Payment Reference (optional, max 100 chars, BRL-092) | Status: Pending/Approved/Rejected/Cancelled (BRL-084/BRL-085/BRL-091) | Pending request already exists for tenant → reject (FR-078, E3); requested plan = current plan → reject (FR-078, E4); missing Payment Phone → reject (FR-078, E2) |
...
| DD-008 | Initial/baseline Credit Score for a new Customer | **Resolved v1.7 (Business Owner Backend Completion, pre-Phase 5)** — baseline 70, per `CreditScoreService` | FR-026 |
| DD-009 | Credit Score point-value catalog and band numeric thresholds | **Resolved v1.7 (Business Owner Backend Completion, pre-Phase 5)** — an exact sign-inversion of the already-approved Risk Level Engine point values (`deendoon/docs/Risk_Level_Engine_v1.0.md`), clamped 0–100; bands 90–100 Excellent / 75–89 Good / 50–74 Fair / 0–49 Poor, per `CreditScoreService` | FR-026 |
...
| DD-011 | Credit Limit Reached (and other) notification re-trigger/suppression policy | **Partially resolved v1.7 (Business Owner Backend Completion, pre-Phase 5)** — Credit Limit Reached now fires only on the transition into over-limit, not on every subsequent recalculation while already over-limit (`CustomerBalanceService::recalculate()`); the general re-trigger/suppression policy for other notification types remains open | FR-028, FR-058 |
...
| DD-016 | Overpayment handling (reject / cap / credit) | **Resolved v1.7 (Business Owner Backend Completion, pre-Phase 5)** — rejected (422); a payment amount exceeding the Debt's remaining_balance is never accepted, capped, or credited. Refund/credit-balance handling remains a future feature, not designed here | FR-034, FR-037 |
| DD-017 | Whether payment may be recorded against a Paid/Cancelled/Written-Off Debt | **Resolved v1.7 (Business Owner Backend Completion, pre-Phase 5)** — rejected (422) | FR-034 |
...
| DD-029 | Document regeneration mechanics (permitted? new number vs. reuse?) | Not specified. **Confirmed deferred to Version 2** (Business Owner Backend Completion, pre-Phase 5, Product Owner Decision) — replace-vs-versioning, numbering, audit behavior, and retention all require design beyond Version 1 scope | FR-052 |
...
| DD-032 | Recovery Rate KPI exact formula | **Resolved v1.7 (Business Owner Backend Completion, pre-Phase 5)** — Collected ÷ Became Due × 100 over the selected period, the same formula as Module 9's existing Collection Rate (`CollectionRateService`) | FR-053 |
...
| DD-036 | Mechanism by which a Collection Activity becomes a Calendar entry (Collection Appointments not a distinct entity) | Not specified. **Confirmed deferred to Version 2** (Business Owner Backend Completion, pre-Phase 5, Product Owner Decision) — would require new schema (an appointment date field on Collection Case), workflow, validation, and notifications beyond Version 1 scope; the Calendar continues to aggregate Debt Due Dates, Promise to Pay Dates, and Reminders only | FR-062 |
