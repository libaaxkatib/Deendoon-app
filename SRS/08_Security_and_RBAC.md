...
| **Scope Baseline** | `01_Project_Overview.md` (Reopened v1.3) · `02_Business_Requirements.md` (Reopened v1.3) · `03_Functional_Requirements.md` (v1.7 — **Module 12 still awaiting its original approval**) · `04_Business_Rules.md` (Reopened v1.3) · `05_UI_UX_Specification.md` (Approved & Frozen, v1.1) · `06_Database_Design.md` (Approved & Frozen, v1.3, PostgreSQL) · `07_API_Design.md` (Approved & Frozen, v1.1) |
...
**Terminology disambiguation — read this before Section 5.** `06_Database_Design.md`'s `roles` table defines a tenant-scoped role literally named `super_admin` — one of the six original RBAC roles (Super Admin, Operations Manager, Collection Officer, Finance, Support, Viewer), approved from the very start of this SRS. Separately, the Professional Collection Requests reopening introduced `deendoon_platform_administrator` — a *different*, non-tenant-scoped actor informally nicknamed "the Deendoon Super Admin" throughout `06` and `07`. These are two different roles that happen to share the word "Super Admin" in casual reference. Confusing them in an authorization check would be a real security bug — e.g., granting a tenant's own Super Admin the platform-level power to review *other tenants'* Professional Collection Requests. This document uses **"Tenant Super Admin"** for the `super_admin` role and **"Deendoon Platform Administrator"** for `deendoon_platform_administrator`, without exception, and recommends the same convention going forward in any implementation artifact.
...
**RBAC roles are exactly the seven already approved** (Section 5) — six tenant-scoped, one platform-level. No new role, no new permission category, and no new actor is introduced here.
...
| Role (`06` `roles.name`) | Tenant-Scoped? | Approved Actor | Typical Capability (default configuration) |
|---|---|---|---|
| `super_admin` (**Tenant Super Admin**) | Yes | BA-001, Business Owner / SME Operator | Full access within the tenant: Customers, Debts, Payments, Collection Cases, Documents, Reports/Dashboard, and Administration (Module 12 — User Administration, Role Assignment, Company Profile, System Preferences, Reference Data, Audit Trail). May submit a Professional Collection Request (FR-072) as any authorized tenant user may; **cannot** review or action another tenant's Request — that remains exclusive to the Deendoon Platform Administrator (BR-042). |
| `operations_manager` | Yes | BA-002, Operations Manager | Day-to-day recovery oversight: Customers, Debts, Recovery Workflow, Payment visibility, Collection Case assignment (FR-041) and management, Reports/Dashboard. Administration (Module 12) is not part of this role's default scope, consistent with "oversees... operations," not platform configuration. |
| `collection_officer` | Yes | BA-003, Collection Officer | Executes follow-up and collection actions on assigned Customers/Debts/Cases — Manual Reminders (FR-030), Promise to Pay (FR-031), Collection Activity (FR-044). FR-041's Exception E2 explicitly restricts Case assignment *to* users holding this role — the one place an FR names a specific role by name, not just "authorized user." |
...
| `support` | Yes | BA-005, Support Staff | Limited operational access — view Customers/Debts, assist with non-financial actions. BA-005 is explicitly described as "not a financial decision-maker" (`02` §2.2) — Payment recording and Credit Limit changes are outside this role's default scope. |
| `viewer` | Yes | BA-006, Viewer | Read-only across every module this role can see at all; no create/update/archive/close action anywhere. Field-level restrictions per FR-008 A1/FR-019 A1 (`03`) apply. |
| `deendoon_platform_administrator` (**Deendoon Platform Administrator**) | **No** | BA-008 | Reviews, transitions the status of, and closes Professional Collection Requests (FR-073, FR-076) across every tenant — the one approved cross-tenant capability in Version 1 (BR-042). Has **no** access to any tenant's Customers, Debts, Payments, or Administration outside what `06` §2's relationship-scoped policy exposes through an actual submitted Request. Not modeled as, and never granted, any of the six tenant roles. |

**Consistency check performed:** every row above traces to an already-approved Business Actor (`02_Business_Requirements.md` §2.2) and an already-approved role name (`06_Database_Design.md` §6.1). No role, and no capability beyond what's textually supported by those two documents, was added.
