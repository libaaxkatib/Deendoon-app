...
| **Version** | 1.5 |
| **Status** | Reopened — Section 5 (Roles & Permissions) amended by the RBAC Architecture Amendment; Section 5/6 amended by the Manual Mobile-Money Subscription Payment Flow Amendment (documentation-consistency correction) |
| **State** | Frozen (pending re-freeze) |
| **Author** | Business Analyst / Solution Architect (Claude) |
| **Approved By** | Product Owner |
| **Last Updated** | 2026-08-19 |
| **Scope Baseline** | `01_Project_Overview.md` (Reopened v1.5) · `02_Business_Requirements.md` (Reopened v1.6) · `03_Functional_Requirements.md` (v1.17 — **Module 12 still awaiting its original approval**) · `04_Business_Rules.md` (Reopened v1.11) · `05_UI_UX_Specification.md` (Reopened, v1.9) · `06_Database_Design.md` (Reopened v1.9 — §6.1 amended, PostgreSQL) · `07_API_Design.md` (Reopened v1.8 — §5.4 amended) |
...
| 1.5 | 2026-08-19 | **Documentation-consistency correction, found during the Manual Mobile-Money Subscription Payment Flow Amendment audit — not a new grant of authority.** Section 5's Platform Administrator row and Section 6's "one approved cross-tenant workflow" line described the Deendoon Platform Administrator's cross-tenant capability only in terms of Professional Collection Requests — stale since the Subscription & Storage Self-Service Catch-Up (`03_Functional_Requirements.md` Revision History 1.13) retroactively documented a second, already-implemented cross-tenant capability (reviewing/approving/rejecting every tenant's Subscription Change Requests and Storage Add-on Requests, FR-084) that this document never named. Both corrected to name it, and Section 5 additionally notes the Platform Payment Destination Number capability (FR-086, Manual Mobile-Money Subscription Payment Flow Amendment). No role, permission, or actual access boundary changed — this corrects what the document says, not what the system does. Scope Baseline updated to cite `03` v1.17, `04` v1.11, `05` v1.9, `06` v1.9, `07` v1.8. | Claude |
...
| `deendoon_platform_administrator` (**Platform Administrator**) | **No** | BA-008 | The sole account type on the Super Admin Web Dashboard. Reviews, transitions the status of, and closes Professional Collection Requests (FR-073, FR-076) across every tenant (BR-042) — the cross-tenant capability originally approved in Version 1. **Amended 2026-08-19 (Manual Mobile-Money Subscription Payment Flow Amendment — documentation-consistency correction, not a new grant):** also reviews, approves, or rejects every tenant's pending Subscription Change Requests and Storage Add-on Requests (`03` FR-084), and sets the platform-level Payment Destination Number (`03` FR-086) — a real, already-implemented cross-tenant capability (Subscription & Storage Self-Service Catch-Up, `03` Revision History 1.13) that this table never named until now; see the note under §6 below. Has **no** access to any tenant's Customers, Debts, Payments, or Administration outside what `06` §2's relationship-scoped policy exposes through an actual submitted Request, or what the Subscription/Storage Approval Center exposes through a submitted Subscription Change Request or Storage Add-on Request. |
...
- **The approved cross-tenant workflows are bounded, not general-purpose.** The Deendoon Platform Administrator's elevated visibility is scoped to exactly the tables and relationship chain `06` §2 defines for Professional Collection Requests, or the `subscription_change_requests`/`storage_addons`/`platform_payment_settings` tables' own `BelongsToTenantOrPlatformAdmin` scoping for the Subscription/Storage Approval Center — it is not a platform-wide superuser grant, and no control in this document extends it further. **Documentation-consistency correction, 2026-08-19:** this section previously described Professional Collection Requests as "the one approved cross-tenant workflow," which became stale once the Subscription & Storage Self-Service Catch-Up (`03` Revision History 1.13) retroactively documented a second, already-implemented one — corrected here to name both rather than silently understate the Platform Administrator's actual approved scope.
