...
/**
 * Bimodal by design (06 §2, BR-042; 08 §5): a tenant user (admin/
 * sales_finance, interim RBAC) may act on their own tenant's Requests; the
 * Deendoon Platform Administrator may view any Request and holds the
 * exclusive authority to transition its status or close it. Tenant-vs-
 * platform-admin scoping itself (which rows are visible at all) is
 * enforced in the controller, since ProfessionalCollectionRequest
 * deliberately has no BelongsToTenant scope to fall back on — this class
 * only gates *actions* on a request the controller has already resolved
 * and confirmed the caller may see.
 */
