...
void main() {
  ...
  setUp(() {
    ...
    when(
      () => mockBiometricAuthService.isSupported(),
    ).thenAnswer((_) async => false);
  });

  testWidgets('shows a retry affordance when settings fail to load', (
    tester,
  ) async {
    when(
  ...
  testWidgets(
    'prefills the business and notification fields with the real settings',
    (tester) async {
      when(
        () => mockRepository.fetchSettings(),
  ...
  testWidgets('does not show a Push Notifications toggle', (tester) async {
    when(
      () => mockRepository.fetchSettings(),
    ).thenAnswer((_) async => _settings);
    ...
    expect(find.text('Payment Notifications'), findsOneWidget);
  });

  testWidgets(
    'shows Coming soon for biometric login when the device does not support it',
    (tester) async {
      when(
        () => mockRepository.fetchSettings(),
      ).thenAnswer((_) async => _settings);
  ...
  testWidgets(
    'Mobile Fix #17: shows an error message and leaves the switch off when a supported '
    'device\'s biometric prompt fails or is cancelled — previously silent',
    (tester) async {
      when(
        () => mockRepository.fetchSettings(),
      ).thenAnswer((_) async => _settings);
      ...
      when(
        () => mockBiometricAuthService.authenticate('Biometric Login'),
      ).thenAnswer((_) async => false);
      ...
      await tester.tap(find.widgetWithText(SwitchListTile, 'Biometric Login'));
      await tester.pumpAndSettle();

      expect(
        find.text('Could not verify your biometrics. Please try again.'),
        findsOneWidget,
      );
      final toggle = tester.widget<SwitchListTile>(
        find.widgetWithText(SwitchListTile, 'Biometric Login'),
      );
  ...
  testWidgets(
    'Mobile Fix #17: a successful biometric prompt turns the switch on with no error shown',
    (tester) async {
      when(
        () => mockRepository.fetchSettings(),
      ).thenAnswer((_) async => _settings);
  ...
  testWidgets(
    'shows a validation error instead of saving an empty default credit limit',
    (tester) async {
      when(
        () => mockRepository.fetchSettings(),
      ).thenAnswer((_) async => _settings);
      ...
      await tester.enterText(
        find.widgetWithText(TextFormField, 'Default Credit Limit'),
        '',
      );
  ...
  testWidgets('shows a validation error for a soft limit threshold above 100', (
    tester,
  ) async {
    when(
      () => mockRepository.fetchSettings(),
    ...
    await tester.enterText(
      find.widgetWithText(TextFormField, 'Soft Limit Warning Threshold (%)'),
      '150',
    );
    await tester.tap(find.widgetWithText(ElevatedButton, 'Save Changes'));
    await tester.pumpAndSettle();
  ...
  testWidgets(
    'switching the language dropdown to Somali updates the whole screen immediately',
    (tester) async {
      when(
        () => mockRepository.fetchSettings(),
      ).thenAnswer((_) async => _settings);
