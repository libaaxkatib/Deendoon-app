...
class _MockReferenceDataRepository extends Mock
...
const _transferReasons = [
  ReferenceDataItem(
    ...
    valueLabel: 'Repeated missed promises',
    sortOrder: 1,
    isActive: true,
  ),
];
...
const _requestedServices = [
  ReferenceDataItem(
    id: '1',
    category: 'requested_service',
    valueLabel: 'Field Visit',
    sortOrder: 1,
    isActive: true,
  ),
];
...
void main() {
  ...
  late _MockProfessionalCollectionRepository
  mockProfessionalCollectionRepository;
  ...
  setUp(() {
    ...
    mockProfessionalCollectionRepository =
        _MockProfessionalCollectionRepository();
    ...
    when(() => mockCaseRepository.fetchHistory('01CASE')).thenAnswer(
      (_) async => const CaseHistory(collectionCaseId: '01CASE', entries: []),
    );
  });

  testWidgets(
    'renders case summary, customer, debt, and collection stage from real data',
    (tester) async {
      when(
        () => mockCaseRepository.fetchCase('01CASE'),
      ...
      expect(
        find.text('COL-0001'),
        findsWidgets,
      ); // AppBar title + Case Summary card
  ...
  testWidgets(
    'a closed case hides action buttons and shows an explanatory note instead',
    (tester) async {
      when(
        () => mockCaseRepository.fetchCase('01CASE'),
  ...
  testWidgets('shows a retry affordance when the case fails to load', (
    tester,
  ) async {
    when(
      () => mockCaseRepository.fetchCase('01CASE'),
  ...
  testWidgets('Attachments opens the case-scoped Attachments screen', (
    tester,
  ) async {
    when(
      () => mockCaseRepository.fetchCase('01CASE'),
    ...
    final router = GoRouter(
      ...
      routes: [
        GoRoute(
          path: '/',
          builder: (_, _) => const CaseDetailScreen(caseId: '01CASE'),
        ),
        GoRoute(
          path: '/cases/01CASE/attachments',
  ...
  testWidgets(
    'submitting Close Case calls the real endpoint with the entered outcome',
    (tester) async {
      when(
        () => mockCaseRepository.fetchCase('01CASE'),
      ).thenAnswer((_) async => _openCase);
      when(
        () => mockCaseRepository.close(
          caseId: '01CASE',
          closureOutcome: 'Paid in full',
        ),
      ).thenAnswer((_) async => _closedCase);
  ...
  group('Notes', () {
    testWidgets('displays existing notes when present', (tester) async {
      ...
      expect(
        find.text('Called customer, promised payment next week.'),
        findsOneWidget,
      );
    ...
    testWidgets(
      'editing pre-fills the current notes and submits the trimmed value',
      (tester) async {
        when(
          () => mockCaseRepository.fetchCase('01CASE'),
        ).thenAnswer((_) async => _openCaseWithNotes);
        when(
          () => mockCaseRepository.updateNotes(
            caseId: '01CASE',
    ...
    testWidgets('clearing notes to empty submits null', (tester) async {
      ...
      when(
        () => mockCaseRepository.updateNotes(caseId: '01CASE', notes: null),
      ).thenAnswer((_) async => _openCase);
    ...
    testWidgets(
      'on an API error, shows the exact backend message and keeps the sheet open',
      (tester) async {
        when(
          () => mockCaseRepository.fetchCase('01CASE'),
        ).thenAnswer((_) async => _openCase);
  ...
  group('Submit to Professional Collection', () {
    ...
    setUp(() {
      ...
      when(
        () => mockReferenceDataRepository.fetchCategory('transfer_reason'),
      ).thenAnswer((_) async => _transferReasons);
      when(
        () => mockReferenceDataRepository.fetchCategory('requested_service'),
    ...
    Future<void> fillValidForm(WidgetTester tester) async {
      ...
      await tester.pumpAndSettle();
      await tester.tap(
        find.text('I confirm the Client Declaration for this hand-off.'),
      );
      await tester.pumpAndSettle();
    }

    testWidgets(
      'on success, submits the exact payload and navigates to the new Request Detail screen',
      (tester) async {
        ...
        when(
          () => mockProfessionalCollectionRepository.submit(
            caseId: '01CASE',
            reasons: ['Repeated missed promises'],
    ...
    testWidgets(
      'leaving no reason selected shows a client-side validation error and never calls the repository',
      (tester) async {
        final router = GoRouter(
          initialLocation: '/',
          routes: [
            ...
            GoRoute(
              path: '/professional-requests/:id',
              builder: (_, _) => const Text('Request Detail'),
            ),
          ],
        );
        await pumpWithRouter(tester, router);
        ...
        expect(
          find.text('Select at least one Reason for Transfer.'),
          findsOneWidget,
        );
        verifyNever(
          () => mockProfessionalCollectionRepository.submit(
    ...
    testWidgets(
      'leaving the declaration unchecked shows a client-side validation error',
      (tester) async {
        final router = GoRouter(
          initialLocation: '/',
          routes: [
        ...
        expect(
          find.text('You must accept the Client Declaration to submit.'),
          findsOneWidget,
        );
        verifyNever(
          () => mockProfessionalCollectionRepository.submit(
    ...
    testWidgets(
      'on a 409 conflict, shows the exact backend message and does not navigate',
      (tester) async {
        when(
          () => mockProfessionalCollectionRepository.submit(
            caseId: '01CASE',
