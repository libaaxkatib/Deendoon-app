...
void main() {
  ...
  test('previewImport delegates to the api and returns the parsed preview', () async {
    ...
    final result = await repository.previewImport(filePath: '/tmp/customers.xlsx', fileName: 'customers.xlsx');

    expect(result.batchId, '01BATCH');
  });

  test('previewImport throws ApiException on failure', () async {
    when(() => mockApi.preview(filePath: any(named: 'filePath'), fileName: any(named: 'fileName'))).thenThrow(
      DioException(
    ...
    expect(
      () => repository.previewImport(filePath: '/tmp/bad.xlsx', fileName: 'bad.xlsx'),
      throwsA(isA<ApiException>().having((e) => e.statusCode, 'statusCode', 422)),
    );
  });

  test('commitImport delegates to the api and returns the parsed result', () async {
    const result = ImportCommitResult(batchId: '01BATCH', status: 'committed', results: [], message: 'Import committed successfully');
    when(() => mockApi.commit(batchId: '01BATCH')).thenAnswer((_) async => result);
  ...
  test('commitImport throws ApiException on an already-committed batch', () async {
    when(() => mockApi.commit(batchId: '01BATCH')).thenThrow(
      DioException(
        requestOptions: RequestOptions(path: '/customers/import/01BATCH/commit'),
        response: Response(
          requestOptions: RequestOptions(path: '/customers/import/01BATCH/commit'),
          statusCode: 422,
          data: {
    ...
    expect(
      ...
      throwsA(isA<ApiException>().having((e) => e.message, 'message', 'This import batch has already been committed.')),
    );
  });
}
