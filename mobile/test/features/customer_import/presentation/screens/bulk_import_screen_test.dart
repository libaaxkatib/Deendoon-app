...
Future<PickedImportFile?> _fakePickerReturning(PickedImportFile? file) => Future.value(file);
...
void main() {
  ...
  testWidgets('picking a supported file shows the Selected File Card', (tester) async {
    await _pumpScreen(
      ...
      picker: () => _fakePickerReturning((path: '/tmp/customers.xlsx', name: 'customers.xlsx', size: 2048)),
    );
  ...
  testWidgets('picking an unsupported file type shows a real client-side rejection', (tester) async {
    await _pumpScreen(
      tester,
      repository: mockRepository,
      picker: () => _fakePickerReturning((path: '/tmp/customers.csv', name: 'customers.csv', size: 100)),
    );

    await tester.tap(find.text('Select Excel file'));
    await tester.pumpAndSettle();

    expect(find.text('Unsupported file type. Only .xlsx and .xls files are supported.'), findsOneWidget);
  });

  testWidgets('tapping Import runs the real preview+commit flow and shows the aggregated summary', (tester) async {
    final preview = ImportPreview(
      ...
      rows: [
        ...
        {
          'row_number': 2,
          'data': {'name': 'John Vendor', 'phone': '254722222222', 'credit_limit': 2000},
          'validation_status': 'valid',
          'validation_errors': null,
          'duplicate_match': null,
        },
        {
          'row_number': 3,
          'data': {'name': 'Existing Person', 'phone': '254733333333', 'credit_limit': 900},
          'validation_status': 'valid',
          'validation_errors': null,
          'duplicate_match': {'customer_id': '01EXISTING', 'name': 'Existing Person'},
        },
        {
          'row_number': 4,
  ...
  testWidgets('shows the empty state when the uploaded file has no rows', (tester) async {
    const preview = ImportPreview(batchId: '01BATCH', status: 'preview', rows: []);
    const commitResult = ImportCommitResult(batchId: '01BATCH', status: 'committed', results: [], message: '');
    when(() => mockRepository.previewImport(filePath: '/tmp/empty.xlsx', fileName: 'empty.xlsx'))
        .thenAnswer((_) async => preview);
    when(() => mockRepository.commitImport(batchId: '01BATCH')).thenAnswer((_) async => commitResult);
    ...
    await _pumpScreen(
      tester,
      repository: mockRepository,
      picker: () => _fakePickerReturning((path: '/tmp/empty.xlsx', name: 'empty.xlsx', size: 512)),
    );
    await tester.tap(find.text('Select Excel file'));
    await tester.pumpAndSettle();
  ...
  testWidgets('shows the real backend error message when the import fails', (tester) async {
    when(() => mockRepository.previewImport(filePath: '/tmp/bad.xlsx', fileName: 'bad.xlsx')).thenThrow(
      const ApiException(
        message: 'The uploaded file could not be read. It may be corrupted or in an unsupported format.',
        statusCode: 422,
      ),
    );
    ...
    await _pumpScreen(
      tester,
      repository: mockRepository,
      picker: () => _fakePickerReturning((path: '/tmp/bad.xlsx', name: 'bad.xlsx', size: 512)),
    );
    await tester.tap(find.text('Select Excel file'));
    await tester.pumpAndSettle();
...
ImportResultRow _toResultRow(Map<String, dynamic> json) => ImportResultRow.fromJson(json);
