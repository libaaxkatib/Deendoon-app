...
Future<void> _pumpScreen(
  ...
  ).thenAnswer((_) async => subscription ?? _subscription);
  if (storage != null) {
...
void main() {
  ...
  testWidgets('shows a retry affordance when storage fails to load', (
    tester,
  ) async {
    when(
      () => mockRepository.fetchStorage(),
  ...
  testWidgets('renders the real storage overview fields', (tester) async {
    await _pumpScreen(
      ...
      storage: _storageWithActiveAddon,
    );
    await tester.pumpAndSettle();
    ...
    expect(
      find.text('1 GB'),
      findsOneWidget,
    ); // base storage allowance (plan-only)
    expect(
      find.text('26 GB'),
      findsOneWidget,
    ); // effective storage allowance (plan + addon)
    expect(find.text('24.00 GB'), findsOneWidget); // remaining storage
  });

  testWidgets('renders active storage add-ons with their real fields', (
    tester,
  ) async {
    await _pumpScreen(
      tester,
    ...
    expect(
      find.text('PAY-A1'),
      findsNothing,
    ); // payment reference isn't shown on active add-ons, only pending ones
  });

  testWidgets(
    'shows an honest empty state when there are no active storage add-ons',
    (tester) async {
      await _pumpScreen(
        tester,
        repository: mockRepository,
        storage: _storageEmpty,
      );
      await tester.pumpAndSettle();
  ...
  testWidgets(
    'no Cancel action is offered when nothing was requested in this session',
    (tester) async {
      await _pumpScreen(
        tester,
        repository: mockRepository,
  ...
  testWidgets('selecting a package reveals the Request Storage Add-on button', (
    tester,
  ) async {
    await _pumpScreen(
      tester,
  ...
  testWidgets(
    'a valid submission calls requestStorageAddon with the exact payload and never claims storage increased',
    (tester) async {
      when(
        () => mockRepository.requestStorageAddon(
          storagePackage: '10gb',
          paymentReference: 'PAY-REF-001',
        ),
      ...
      expect(
        find.text(
          'Storage Add-on request for 10GB submitted — pending Platform Administrator approval.',
        ),
        findsOneWidget,
      );
      ...
      expect(
        find.text('1 GB'),
        findsWidgets,
      ); // base + effective both 1 GB in this fixture
  ...
  testWidgets(
    ...
    (tester) async {
      when(
        ...
        const ApiException(
          message:
              'A pending Storage Add-on Request already exists for this tenant.',
          statusCode: 409,
        ),
      );
  ...
  testWidgets(
    'cancelling the just-created pending request calls the real endpoint with the real id',
    (tester) async {
      when(
        () => mockRepository.requestStorageAddon(
          storagePackage: '10gb',
      ...
      when(
        () => mockRepository.cancelStorageAddon('addon-2'),
      ).thenAnswer((_) async => _cancelledAddonResponse);
  ...
  testWidgets(
    'reconciliation is scoped by id: refreshing to a DIFFERENT active add-on does not clear the pending card',
    (tester) async {
      var fetchCount = 0;
  ...
  testWidgets(
    'reconciliation clears the pending card once the same id appears in the active add-ons list',
    (tester) async {
      var fetchCount = 0;
      when(() => mockRepository.fetchStorage()).thenAnswer((_) async {
        fetchCount++;
