  ...
  final router = GoRouter(
    ...
    routes: [
      GoRoute(
        path: '/',
        builder: (_, _) =>
            const ProfessionalCollectionDetailScreen(requestId: '1'),
      ),
      GoRoute(
        path: '/cases/:id',
        builder: (_, state) =>
      ...
      GoRoute(
        ...
        builder: (_, _) => const Text('Messages Screen'),
      ),
      GoRoute(
        path: '/professional-requests/:id/documents',
        builder: (_, _) => const Text('Documents Screen'),
      ),
      GoRoute(
      ...
      GoRoute(
        ...
        builder: (_, _) => const Text('Timeline Screen'),
      ),
    ],
  );
...
void main() {
  ...
  testWidgets('renders request summary with real fields', (tester) async {
    ...
    expect(
      find.text('01USER'),
      findsWidgets,
    ); // Submitted By + Declaration Accepted By
    expect(find.text('02USER'), findsOneWidget);
  });

  testWidgets(
    'renders reasons, requested services, notes, and declaration record from real data',
    (tester) async {
      when(
        () => mockRepository.fetchRequest('1'),
      ).thenAnswer((_) async => _request);
      ...
      expect(
        find.text('Customer stopped responding after the third follow-up.'),
        findsOneWidget,
      );
  ...
  testWidgets(
    'shows honest empty states when reasons/services/notes are absent',
    (tester) async {
      ...
      when(
        () => mockRepository.fetchRequest('1'),
      ).thenAnswer((_) async => bareRequest);

      await _pumpScreen(tester, repository: mockRepository);
      await tester.pumpAndSettle();

      expect(
        find.text('No reasons recorded for this Request.'),
        findsOneWidget,
      );
      expect(
        find.text('No requested services recorded for this Request.'),
        findsOneWidget,
      );
  ...
  testWidgets('shows a retry affordance when the request fails to load', (
    tester,
  ) async {
    when(
      () => mockRepository.fetchRequest('1'),
    ...
    expect(
      find.text('Could not load this Professional Collection Request.'),
      findsOneWidget,
    );
    expect(find.text('Retry'), findsOneWidget);
  });

  testWidgets('View Collection Case opens the real Case Detail screen', (
    tester,
  ) async {
    when(
      () => mockRepository.fetchRequest('1'),
