...
Future<void> _pumpScreen(
  WidgetTester tester,
  _MockReminderRepository repository,
) async {
  final router = GoRouter(
    ...
    routes: [
      GoRoute(
        path: '/send',
        builder: (_, _) => const MessagePreviewScreen(reminderId: '1'),
      ),
    ],
  );
...
void main() {
  ...
  testWidgets(
    'renders the 7 default templates in the approved document order, not alphabetically',
    (tester) async {
      ...
      for (var i = 1; i < positions.length; i++) {
        expect(
          ...
          reason: '"${order[i]}" should not appear before "${order[i - 1]}"',
        );
      }
    },
  );
  ...
  testWidgets(
    'Send via WhatsApp is disabled until a template has rendered, then enabled',
    (tester) async {
      ...
      when(
        ...
        (_) async => {
          'rendered_text':
              'Mudane/Marwo Hodan Store, waxaad leedahay deyn dhan 200.00.',
      ...
      final buttonBefore = tester.widget<ElevatedButton>(
        find.widgetWithText(ElevatedButton, 'Send via WhatsApp'),
      );
