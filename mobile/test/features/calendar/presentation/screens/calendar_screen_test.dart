...
Future<void> _pumpScreen(
  WidgetTester tester,
  _MockCalendarRepository repository,
) async {
...
void main() {
  ...
  testWidgets('fetches the current month range and shows its label', (
    tester,
  ) async {
    when(
      () => mockRepository.fetchEntries(
        from: _iso(monthStart),
        to: _iso(monthEnd),
      ),
    ).thenAnswer(
      (_) async => CalendarData(
        from: _iso(monthStart),
        to: _iso(monthEnd),
        entries: const [],
      ),
  ...
  testWidgets('shows the empty state for today when there are no entries', (
    tester,
  ) async {
    when(
      () => mockRepository.fetchEntries(
  ...
  testWidgets('renders an agenda tile for a real entry due today', (
    tester,
  ) async {
  ...
  testWidgets(
    'tapping Next month re-fetches with next month\'s real date range',
    (tester) async {
      ...
      when(
        () => mockRepository.fetchEntries(
          from: _iso(nextMonthStart),
          to: _iso(nextMonthEnd),
        ),
      ).thenAnswer(
        (_) async => CalendarData(
          from: _iso(nextMonthStart),
      ...
      expect(
        find.text(
          '${_monthNames[nextMonthStart.month - 1]} ${nextMonthStart.year}',
        ),
        findsOneWidget,
      );
    },
  ...
  testWidgets('shows a retry affordance when the calendar fails to load', (
    tester,
  ) async {
    when(
      () => mockRepository.fetchEntries(
