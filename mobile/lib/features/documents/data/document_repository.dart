...
final documentRepositoryProvider = Provider<DocumentRepository>(
  (ref) => DocumentRepository(ref.read(documentApiProvider)),
);
...
class DocumentRepository {
  ...
  Future<DocumentSummary> fetchDocument(String id) =>
      _guard(() => _api.show(id));

  Future<List<int>> downloadDocument(String id) =>
      _guard(() => _api.download(id));

  Future<List<DocumentEvent>> fetchHistory(String id) =>
      _guard(() => _api.history(id));

  Future<SentMessage> shareDocument({
    required String id,
    required String channel,
    required String templateId,
  }) => _guard(
    () => _api.share(id: id, channel: channel, templateId: templateId),
  );
