...
final analyticsRepositoryProvider = Provider<AnalyticsRepository>(
  (ref) => AnalyticsRepository(ref.read(analyticsApiProvider)),
);
...
class AnalyticsRepository {
  ...
  Future<CollectionAnalytics> fetchCollectionAnalytics({
    required String dateFrom,
    required String dateTo,
  }) => _guard(
    () => _api.collectionAnalytics(dateFrom: dateFrom, dateTo: dateTo),
  );

  Future<RiskDistribution> fetchRiskDistribution() =>
      _guard(() => _api.riskDistribution());

  Future<CollectionsTrend> fetchCollectionsTrend({
    required String dateFrom,
    required String dateTo,
  }) => _guard(() => _api.collectionsTrend(dateFrom: dateFrom, dateTo: dateTo));

  Future<AgingAnalysis> fetchAgingAnalysis() =>
      _guard(() => _api.agingAnalysis());

  Future<CustomerPage> fetchReportCustomers({
    required int page,
    String? customerStatus,
    String? riskLevel,
  }) => _guard(
    () => _api.reportCustomers(
      page: page,
  ...
  Future<CollectionCasePage> fetchReportCollectionCases({
    required int page,
    String? status,
  }) => _guard(() => _api.reportCollectionCases(page: page, status: status));

  Future<PaymentPage> fetchReportPayments({
    required int page,
    String? dateFrom,
    String? dateTo,
  }) => _guard(
    () => _api.reportPayments(page: page, dateFrom: dateFrom, dateTo: dateTo),
  );

  Future<CustomerPage> fetchReportCreditRisk({
    required int page,
    String? riskLevel,
  ...
  Future<List<int>> exportReport({
    ...
    () => _api.export(reportType: reportType, format: format, filters: filters),
  );
