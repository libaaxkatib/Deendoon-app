...
final debtApiProvider = Provider<DebtApi>(
  (ref) => DebtApi(ref.read(dioProvider)),
);
...
class DebtApi {
  ...
  Future<void> archive(String id) async {
    await _dio.post('debts/$id/archive');
  }

  /// `->withTrashed()` on the backend route — reachable for an
  /// already-archived (soft-deleted) debt id.
  Future<Debt> restore(String id) async {
    final response = await _dio.post('debts/$id/restore');
    return Debt.fromJson(response.data['data'] as Map<String, dynamic>);
  }
  ...
  Future<PromiseToPay> promiseToPay({
    ...
    final response = await _dio.post(
      'debts/$debtId/promise-to-pay',
      data: {'promised_date': promisedDate},
    );
  ...
  Future<List<PromiseToPay>> promiseToPayHistory(String debtId) async {
    ...
    final data = response.data['data'] as List<dynamic>;
    return data
        .map((e) => PromiseToPay.fromJson(e as Map<String, dynamic>))
        .toList();
  }
  ...
  Future<DebtSaveResult> store({
    ...
    final response = await _dio.post(
      ...
      data: {'amount': amount, 'due_date': dueDate, 'notes': notes},
    );
    final data = response.data['data'] as Map<String, dynamic>;
    return (
      debt: Debt.fromJson(data['debt'] as Map<String, dynamic>),
      warning: data['warning'] != null
          ? CreditLimitWarning.fromJson(data['warning'] as Map<String, dynamic>)
          : null,
    );
  }
  ...
  Future<Debt> update({
    ...
    final response = await _dio.put(
      'debts/$debtId',
      data: {'due_date': dueDate, 'notes': notes},
    );
    return Debt.fromJson(response.data['data'] as Map<String, dynamic>);
  }
  ...
  Future<Debt> logWhatsAppReminder({
    required String debtId,
    String? details,
  }) async {
    final response = await _dio.post(
      'debts/$debtId/reminders/whatsapp',
  ...
  Future<Debt> logCallReminder({
    required String debtId,
    String? details,
  }) async {
    final response = await _dio.post(
      'debts/$debtId/reminders/call',
      data: {'details': details},
    );
    return Debt.fromJson(response.data['data'] as Map<String, dynamic>);
  }
